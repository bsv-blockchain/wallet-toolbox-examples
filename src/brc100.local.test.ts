...
import { Setup, SetupWallet, StorageKnex } from '@bsv/wallet-toolbox'
import { Knex, knex as makeKnex } from 'knex'

// Helper to create SQLite Knex connection
function createSQLiteKnex(filename: string): Knex {
...
describe('BRC-100 Wallet Operations', () => {
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      const dbFilePath = `wallet_test_brc100.local.sqlite`
      console.log(`🔄 Creating local SQLite database: ${dbFilePath}`)
      const knex = createSQLiteKnex(dbFilePath)
      knexConnections.push(knex) // Track for cleanup
      ...
      await localStorage.migrate(dbFilePath, dbFilePath)
      console.log('✅ Database migrations completed')
      await setup.storage.addWalletStorageProvider(localStorage)
      console.log('✅ Local SQLite storage provider added to wallet')

      // Local storage doesn't require authentication verification
  ...
  afterAll(async () => {
    ...
    // Properly destroy the wallet and storage to close any open connections
    try {
    ...
    // Clean up knex connections
    for (const knex of knexConnections) {
      try {
        await knex.destroy()
  ...
  describe.skip('Blockchain Info', () => {
    test('getHeight - should fetch current block height', async () => {
      // Skipped for local storage tests - requires external blockchain API
    }, 10000)
